...
export default function OpenCodeWebView({ visible = true }: Props) {
  ...
  // Start on mount — check if already running, otherwise launch
  useEffect(() => {
    getOpenCodeWebStatus().then((res) => {
      if (res.status === 'running' && res.port) {
        setPort(res.port);
        setStatus('running');
      } else {
    ...
    <div className="tc-layout">
      <div className={`tc-sidebar ${sidebarOpen ? '' : 'collapsed'}`}>
        <div className="tc-sidebar-header">
          ...
          <span className="tc-sidebar-title">OpenCode</span>
        ...
        <div className="tc-sidebar-section">
          ...
          <div className="tc-sidebar-tips">
            ...
            Type your request at the prompt. Use /help for available commands and skills.
          </div>
        </div>
      </div>
        ...
        <div className="tc-terminal-wrapper">
          {status === 'loading' && (
            <div className="tc-loading">
              ...
              <div className="tc-loading-text">Starting OpenCode...</div>
            </div>
          )}
          {status === 'error' && (
            <div className="tc-loading">
